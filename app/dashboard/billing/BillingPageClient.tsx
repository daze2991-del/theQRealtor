...
import { PRICING_TIERS, PRICING_CLARIFIER, pricingTierConfig, isPricingTier, type PricingTier } from '../../../lib/pricing'
...
// Human label for the current-plan badge. Starter/Pro read their real name
// from the pricing catalog; the grandfathered/fallback plans keep their
// existing wording. Never says "unlimited" — see limitLabel() below.
...
// Customer-facing limit copy. A null limit is phrased as "All your active
// <noun>s" rather than "Unlimited" — same rule Phase 5/6 apply everywhere else.
function limitLabel(n: number | null, noun: string): string {
  return n === null ? `All your active ${noun}s` : `${n} active ${noun}${n === 1 ? '' : 's'}`
}

export default function BillingPageClient({ paidPlansEnabled }: { paidPlansEnabled: boolean }) {
  ...
  const badge = (p: PlanId): React.CSSProperties => ({
  ...
  const currentLimits = planConfig(plan)
              ...
              <div style={card}>
                <div style={row}>
                  <div>
                    ...
                    <span style={badge(plan)}>{planDisplayName(plan)}</span>
                  </div>
                  {subscription && (
                    ...
                    <div>
                      ...
                      <span style={{ fontSize: 14, color: '#9CA3AF' }}>
                        {subscription.interval === 'year'
                          ? 'Yearly'
                          : isPricingTier(plan) ? `Monthly (${pricingTierConfig(plan).displayPrice})` : 'Monthly'}
                      </span>
                    </div>
                    <div>
                ...
                {paidPlansEnabled ? (
                  <>
                    <p style={{ color: '#9CA3AF', fontSize: 14, marginBottom: 20 }}>
                      Choose a plan to unlock more listings and signs.
                    </p>
                      ...
                      {PRICING_TIERS.map(tierKey => {
                        ...
                        return (
                          <div key={tierKey} style={{ background: `${C.purple}10`, border: `1px solid ${C.border}`, borderRadius: 10, padding: 20 }}>
                            ...
                            <div style={{ fontSize: 22, fontWeight: 700, color: C.purpleL, marginBottom: 8 }}>{cfg.displayPrice}</div>
                            <div style={{ fontSize: 12, color: C.muted, marginBottom: 10, lineHeight: 1.5 }}>{cfg.copy}</div>
                            <button onClick={() => handleCheckout(tierKey)} disabled={checkingOut}
                              style={{ ...btnPrimary, marginBottom: 0, opacity: checkingOut ? 0.7 : 1, cursor: checkingOut ? 'not-allowed' : 'pointer' }}>
                              {checkingOut ? 'Redirecting…' : `Upgrade to ${cfg.displayName}`}
                            </button>
                          </div>
                        )
                      })}
                  ...
                  <>
                    <p style={{ color: '#9CA3AF', fontSize: 14, marginBottom: 14 }}>
                      {hasPaidPlan
                        ? <>You're a <strong style={{ color: '#4ade80' }}>{planDisplayName(plan)}</strong>. You have free full platform access during the beta testing period. No charge and no payment method required while in beta.</>
                        : <>You're on the <strong style={{ color: C.text }}>Free plan</strong>.</>
                      }
                    </p>
                    <div style={{ display: 'flex', gap: 24 }}>
                      <div>
                        <span style={{ ...label, display: 'block', marginBottom: 4 }}>Listings</span>
                        <span style={{ fontSize: 14, color: '#9CA3AF' }}>{limitLabel(currentLimits.maxActiveListings, 'listing')}</span>
                      </div>
                      <div>
                        <span style={{ ...label, display: 'block', marginBottom: 4 }}>Signs</span>
                        <span style={{ fontSize: 14, color: '#9CA3AF' }}>{limitLabel(currentLimits.maxActiveSigns, 'sign')}</span>
                      </div>
                    </div>
                  </>
                )}

                {paidPlansEnabled && hasPaidPlan && (
                  <>
                    <div style={divider} />
